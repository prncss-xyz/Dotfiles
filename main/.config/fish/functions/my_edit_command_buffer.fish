function my_edit_command_buffer --description 'Edit the command buffer in an external editor'
    set -l f (mktemp)
    or return 1
    if set -q f[1]
        command mv $f $f.fish
    ...
    switch $basename
        ...
        case nvim
            # set -a editor +$line +"norm $col|" $f
            if test \( $col -eq 1 \) -a \( $line -eq 1 \)
                set -a editor +"Telescope luasnip" +$line +"norm $col|" $f
            else
                set -a editor +$line +"norm $col|" $f
            end
