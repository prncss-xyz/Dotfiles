...
function M.command(name, opts, rhs)
	...
	local types = (opts.types and type(opts.types) == "table") and table.concat(opts.types, " ") or ""
...
function M.echo(msg, hl)
	...
	assert(
		...
		string.format("message should be a string or list of strings not a %s", msg_type)
	)
