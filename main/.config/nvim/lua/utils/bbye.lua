...
local function new(force)
  ...
  create_autocmd('BufWipeout', {
    buffer = 0,
    callback = function()
      vim.api.nvim_buf_delete(empty_buffer, {})
    end,
...
function bbye.delete(action, force, buffer, mods, focus_id)
  local buffer_number = type(buffer) == 'string' and bufnr(buffer)
    or buffer
    or get_current_buf()
  mods = mods or ''
  ...
  require('utils.std').reverse(window_ids)

  for _, window_number in ipairs(window_ids) do
  ...
  if buflisted(buffer_number) == 1 and buffer_number ~= get_current_buf() then
    local no_errors = pcall(function()
      command(
        mods .. ' ' .. action .. (force and '!' or '') .. ' ' .. buffer_number
      )
    end)
...
function bbye.close()
  if vim.bo.buftype == '' then
    bbye.bdelete(true, vim.api.nvim_get_current_buf(), nil, nil)
  end
end
